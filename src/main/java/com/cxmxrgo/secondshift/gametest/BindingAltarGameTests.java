 ...
 * safety net.
 *
 * <p>Covers: {@code stillValid} staying {@code true} while the altar, the job block above it,
 * and player proximity are all valid (SC4 baseline), and flipping to {@code false} after each of
 * altar-break / job-block-removal / distance failure (SC4's explicit acceptance criterion — a
 * malformed/stale menu interaction is rejected server-side without a crash). Also covers the
 * no-job-block interaction path throwing no exception and leaving the altar unsocketed (D-11,
 * ALTAR-03), and {@link ProfessionResolver} correctly mapping a real job-site block while
 * ignoring a non-POI block (D-08/D-09/D-10).
 *
...
public final class BindingAltarGameTests {
    ...
    private static final BlockPos FAR_JOB_POS = new BlockPos(4, 3, 4);
    ...
    public static void binding_altar_menu_open_stillvalid_true(GameTestHelper helper) {
        ...
        helper.setBlock(ALTAR_POS.above(), Blocks.CARTOGRAPHY_TABLE);
        ...
        helper.assertTrue(menu.stillValid(player),
                "stillValid must be true while altar + job block + proximity are all valid");
        helper.succeed();
    }
    ...
    public static void binding_altar_no_job_block_no_menu_no_crash(GameTestHelper helper) {
        helper.setBlock(ALTAR_POS, ModBlocks.SOUL_ALTAR.get());
        // Deliberately no block above ALTAR_POS.
        ...
        BlockPos absAltarPos = helper.absolutePos(ALTAR_POS);
        helper.assertTrue(
                helper.getLevel().getBlockEntity(absAltarPos) instanceof SoulAltarBlockEntity be && be.isEmpty(),
                "the altar must remain unsocketed when no job block is present (D-11)");
        helper.succeed();
    }

    @GameTest(template = "empty")
    public static void profession_resolver_maps_real_job_site(GameTestHelper helper) {
        helper.setBlock(FAR_JOB_POS, Blocks.CARTOGRAPHY_TABLE);

        Optional<VillagerProfession> resolved =
                ProfessionResolver.fromAbove(helper.getLevel(), helper.absolutePos(FAR_JOB_POS.below()));

        helper.assertTrue(resolved.equals(Optional.of(VillagerProfession.CARTOGRAPHER)),
                "a real job-site block must resolve to its mapped profession, got " + resolved);
        helper.succeed();
    }

    @GameTest(template = "empty")
    public static void profession_resolver_ignores_non_poi_block(GameTestHelper helper) {
        helper.setBlock(FAR_JOB_POS, Blocks.STONE);

        Optional<VillagerProfession> resolved =
                ProfessionResolver.fromAbove(helper.getLevel(), helper.absolutePos(FAR_JOB_POS.below()));

        helper.assertTrue(resolved.isEmpty(),
                "a non-POI block must resolve to no profession, got " + resolved);
        helper.succeed();
    }
}
